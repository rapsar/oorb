...
for t = unique(t1)'
    ...
    idx1 = find(t1==t);
    idx2 = find(t2==t+dk);
    alpha1 = xy2alpha(xyt1(idx1,1:3),frameDim);
    alpha2 = xy2alpha(xyt2(idx2,1:3),frameDim);
    alpha1(:,end+1) = idx1;
    alpha2(:,end+1) = idx2;
    ...
    if ~isempty(alpha1) && ~isempty(alpha2)
        ...
        % matchedAlpha1t = vertcat(matchedAlpha1t,[ma1 repmat(t,size(ma1,1),1)]);
        % matchedAlpha2t = vertcat(matchedAlpha2t,[ma2 repmat(t,size(ma1,1),1)]);
        matchedAlpha1t = vertcat(matchedAlpha1t,ma1); % [a1 a2 a3 t1 idx1]
        matchedAlpha2t = vertcat(matchedAlpha2t,ma2);
...
for i = 1:N1    
    for j = 1:N2
        ...
        % line
        if abs(dot(alpha1(i,1:3)',t))>0.99 || abs(dot(beta2(j,:)',t))>0.99
            cij(i,j) = Inf;
